...
Executed 2026-08-09 on branch `one-pipeline` (five commits on top of `f20b05e`). Build and the full test suite (`xcodebuild test`, plus `scripts/chalant-hook-selftest`) were run green after every phase. Everything below supersedes the corresponding sections of the audit above; the audit body was left as written, as the record of the before-state. One audit claim was checked against the code and held up (nothing contradicted the code); the fused doc comments found in §8 turned out to be two separate displaced blocks, both fixed.

## Phase 0 — hygiene (commit `679a40a`)

- The two stale "a decision on PermissionRequest is ignored" comments now state the corrected finding (the 2026-08-06 test used the PreToolUse field name; the event obeys `decision.behavior`): `ActivityServer.swift` (the `/prompt` route header) and `TerminalPromptCard`'s doc in `SessionCards.swift`. `ApprovalCard`'s doc block, found fused onto `TerminalPromptCard`'s, was moved back above its own struct.
- `disarm()` (`HookInstall.swift`) now removes our inner hooks from each `PreToolUse` entry instead of removing only entries made entirely of them, so a hand-built entry mixing Chalant's hook with another tool's can actually be disarmed; the other tool's half is kept. Regression test added (`ArmingTests`).

## Phase 1 — one pipeline (commit `fe760cb`)

- `POST /hook/pre-tool-use` is now the gate: parses the call, consults the arming rules, and suspends the request on `PendingDecisionStore` exactly like `/hook/permission-request` — same store, same hangup detection (`watchForHangup`), same session-end release. The pipeline lives in `ActivityServer.settleGate(call:patience:rules:)` so tests reach it without a socket.
- `HookInstall.arm()` writes an http entry (`matcher "*"`, `url http://127.0.0.1:<port>/hook/pre-tool-use`, `timeout 600`, literal `Authorization: Bearer <token>`), resolving the live port/token like `armPrompts`. It removes the legacy command entry in the same write; `holdsToolCalls` still recognizes the legacy shape so `publish()` refreshes/upgrades armed installs on every bind — an install armed before this migrates on next app launch with no button press.
- Deleted: the `PreToolUse` block of `scripts/chalant-hook` (an old command entry now falls through and exits silently), `POST /permission`, `GET /permission/<id>`, `DELETE /permission/<id>`, and `SessionStore.collectDecision`. Kept: `GET /permissions` and `PUT /permission/<id>` (the `chalant pending`/`approve`/`deny` CLI path, which now resolves the held hook through `onDecided`).
- `Approval.patience` defaults to 600 (the hook's own timeout, carried in the payload) — the 25-second polling patience is gone.
- New `GatePipelineTests`: allow, deny, silence (no rule), hangup, session-end release, one-hold-per-session.
- Files: `ActivityServer.swift`, `HookInstall.swift`, `SessionStore.swift`, `scripts/chalant-hook`, `DashboardSections.swift` (snippet now shows the http shape), `ArmingTests.swift`, `PromptGateTests.swift`, `SessionStoreTests.swift`, `GatePipelineTests.swift` (new).

## Phase 2 — one rule store (commit `d3221b0`)

- `Grant` carries both shapes: `expires: Date?` (nil never runs out) and `repo: ""` (everywhere). `mustAsk` still vetoes every grant, so neither shape can cover the always-ask list. `PolicyStore.grant(pattern:repo:for:)` takes `TimeInterval?`.
- `settleGate` consults grants first (`evaluateGrantsOnly`) and answers allow outright, written to the audit, before the hold rules run. `holdForApproval` lost its `exceptions` parameter; only rules decide a hold.
- The island's "Always allow X" button writes a forever/global grant through `PolicyStore`; the timed menu stays repo-scoped and gained an "Always, everywhere" item. The card's grant closure is now `(pattern, repo, duration?)`.
- `PolicyStore.migrateLegacyExceptions(from:)` runs once at launch (`NotchViewModel`, before the server starts): each UserDefaults exception becomes a forever/global grant, deduped, and the key is emptied. `SessionStore.approvalExceptionsKey` / `approvalExceptions()` / `addException()` are deleted; the dashboard's exceptions editor is gone and "Standing permissions" renders everywhere/until-revoked rows.
- Acceptance test: `testAlwaysAllowIsHonoredOnTheVeryNextMatchingCall` — the tap writes the grant and the very next matching call is answered allow with no card.
- Files: `PolicyEngine.swift`, `ActivityServer.swift`, `SessionStore.swift`, `SessionCards.swift`, `AgentSessions.swift`, `SessionRoom.swift`, `DashboardSections.swift`, `NotchViewModel.swift`, `PolicyTests.swift`, `GatePipelineTests.swift`, `PromptGateTests.swift`, `SessionStoreTests.swift`.

## Phase 3 — question cards via hooks (commit `54852b3`)

- `armPrompts` installs a fifth entry: `PreToolUse` with `matcher "AskUserQuestion"`, http, `timeout 30`, pointing at `POST /hook/ask-user`. It rides the prompts switch and is removed by `disarmPrompts`; it coexists with the gate's own matcher-`*` entry (proven by test).
- The route parses `tool_input.questions` (`HookPayload.questions(from:)`: header, text, options as aligned label/description pairs, multiSelect, `field = "q<i>"`), surfaces an `Ask` flagged `intercepted`, and suspends on `PendingAnswerStore` — the question-shaped twin of `PendingDecisionStore` (the audit's target list said PendingDecisionStore; the answer carries text, not just allow/deny, so it holds on the store built for answers, same continuation discipline).
- A tap answers with exactly: `{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"deny","permissionDecisionReason":"User answered via Chalant: <answer>. Treat this as the final answer and continue. Do not re-ask."}}` (bundles join as `Header: answer; Header: answer`). No answer within 20 seconds (`ActivityServer.questionPatience`, hook timeout 30 as backstop) → empty 200, terminal picker untouched. "Answer in terminal" declines to the same fallthrough.
- The card renders numbered options with descriptions and the free-text field; multi-select answers now travel whole (`wire()` joins picks instead of taking the first).
- `PermissionRequest` (`hold(_:on:event:)`) and the gate (`settleGate`) both stand aside for `AskUserQuestion`, so a question is never eaten as a yes/no.
- Deleted: the transcript scrape (`SessionDiscovery.NativeAsk`, `pendingNativeAsk`, its parser and tool_result clearing), the queue-a-pick outbox path (`AskCard.queue`, `queuedOutcome`), and `Ask.native`. The scripted `chalant ask` and MCP elicitation flows are unchanged.
- Files: `HookGate.swift`, `HookInstall.swift`, `ActivityServer.swift`, `SessionStore.swift`, `SessionDiscovery.swift`, `SessionCards.swift`, `AgentSessions.swift`, `SessionRoom.swift`, plus `GatePipelineTests`, `PromptGateTests`, `SessionStoreTests`.

## Phase 4 — two small wins (commit `21ef89b`)

- `/hook/stop` reads `last_assistant_message` from its own payload and records it via `SessionStore.noteLastWords` (trimmed, capped at 400, never blanking real words with an empty turn). `Finished.lastMessage`, the rail summary, and the came-to-rest announcement all feed from it. The transcript-derived source is deleted (`SessionDiscovery` text-block fold, `TrackedFile.lastMessage`, `upsert`'s `lastMessage` parameter).
- `scripts/chalant-hook` branches on `notification_type` (sanitized to one shell-safe word): `agent_needs_input` → "<folder> needs input" (needs-input pill), `agent_completed` → "<folder> finished" (done pill), `permission_prompt` unchanged (generic pill + separately-posted subject), anything unrecognized keeps the generic pill. Selftest covers all three.
- Files: `ActivityServer.swift`, `SessionStore.swift`, `SessionDiscovery.swift`, `scripts/chalant-hook`, `scripts/chalant-hook-selftest`, `SessionRoomDataTests.swift`, `SessionStoreTests.swift`.

## Open risks

- **Unarmed installs lose two things they used to get free.** With the prompts switch off: (1) no question cards at all — the transcript-scraped ask card is gone and interception needs the hook; (2) no finished-row summaries and a degraded came-to-rest announcement — `lastMessage` only arrives via the Stop http hook, and with it absent the announcement falls back to "came to rest in idle", whose dedup-by-words can swallow later turns' announcements. Both are the cost of "delete the old path" rather than "keep both"; the mitigation is that `armPrompts` is one button and is auto-refreshed once armed.
- **The question entry arrives on existing armed installs silently.** `publish()` refreshes `armPrompts` entries on every launch, so machines with the switch on (this one included) gain the `PreToolUse`/`AskUserQuestion` entry on next launch without a new consent step. Defensible (it is the same switch's feature), but it is a behavior change shipped by refresh.
- **The deny-with-answer contract is a prompt-engineering contract.** Claude Code shows `permissionDecisionReason` to the model; the "Treat this as the final answer" wording is tested for shape, not for model behavior. If a model re-asks despite it, the question re-arrives and is held again — annoying, not destructive. Unverified against a live session in this run.
- **`agent_needs_input` / `agent_completed` field values are unverified on the wire.** The branch keys on `notification_type` values named in the migration spec; a live capture (the `hookPayloadLog` switch) should confirm Claude Code's actual strings. Unrecognized values degrade to the old generic pill, so the failure mode is the status quo.
- **Gate patience is now 600 s where the polling shim gave up at 25 s.** A held call whose human walked away suspends the agent for up to the hook timeout (hangup/stop still release it early). The card's countdown tells the truth, but the old behavior effectively self-released at 25 s.
- **Migration one-way doors.** Grants written by the new build (`expires` optional/absent) will not decode in an old build's `PolicyStore` (its `expires` is non-optional), and a migrated machine's exceptions key is emptied — rolling back to 1.8.2 loses "Always allow" state. Same class of caveat for settings.json: the legacy command gate entry is upgraded to http on first launch of the new build; rolling back leaves the http entry, which the old build's `holdsToolCalls` does not recognize (the gate would read as disarmed there, though `chalant-hook` in the old bundle would still answer its polling routes against a server that no longer has them — it fails open, silently).
- **Not touched, per instruction:** phone push, the Codex path (still unverified end to end), Cursor session handling (`/hook/gate`, the shim, `default`-row collapse). The Cursor pipeline still runs through `PolicyEngine.evaluate` inside `/hook/gate` and was not consolidated with `settleGate`; the two hold flows remain parallel code paths.
